        ...
        if (namespaceRewrites) {
            ...
            prefix += names.join(', ') + [
                ...
                "if (typeof Ext === 'undefined') {\n" +
                    "\tthis.Ext = {};\n" +
                "}\n" +
                "Ext.sandboxName = '", namespaceRewrites[0].to, "';\n" +
                "Ext.isSandboxed = true;\n",
            ].join("");
            if (settings) {
                prefix += "Ext.buildSettings = " + JSON.stringify(settings) + ";\n";
            }
...
//            }
        ...
        includes.forEach(function(include) {
            ...
            if (!Fs.exists(jsbDir + path)) {
                if (Platform.isUnix) {
                    ...
                    if (!gotFile && ((path.indexOf('app.js') == -1) || verbose)) {
                        Logger.log("[ERROR] File '" + jsbDir + path + "' is either not existent or unreadble");
                    }
                } else {
                    // the default 'app.js' file is assumed to be present and will attempt
                    // to load, even if the application does not use it (portal example)
                    if ((path.indexOf('app.js') == -1) || verbose) {
                        Logger.log("[ERROR] File '" + jsbDir + path + "' is either not existent or unreadble");
                    }
                }
            }
    ...
    getContent: function(file, callNum) {
        ...
        try {
            ...
            if (Platform.isWindows && callNum < 5) {
                ...
                Logger.log("[ERROR] failed to load resource '" + file + "'");
            }
        }
        return content;
    },
    ...
    getIncludePath : function(include) {
        var path = include.path.replace(/\//g, Fs.sep) + (include.name || include.text || '');
